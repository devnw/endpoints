...
func (history *JobHistory) create(user domain.User, permission domain.Permission) (generalResp *GeneralResp, status int, err error) {
	...
	if err == nil {
		if configs != nil {
			if history.StatusID > 0 && len(history.ConfigID) > 0 {
				...
				if err == nil {
					...
					generalResp.Response = fmt.Sprintf("Job history created using config id [%s] with status id [%d]",
						history.ConfigID, history.StatusID)
...
func (history *JobHistory) delete(user domain.User, permission domain.Permission) (generalResp *GeneralResp, status int, err error) {
	...
	if len(history.JobHistID) > 0 {
		_, _, err = Ms.UpdateJobHistoryStatusDetailed(history.JobHistID, 5, sord(user.Username()))
		if err == nil {
			status = http.StatusOK
			generalResp.Response = fmt.Sprintf("job history %s deleted", history.JobHistID)
		} else {
...
func (history *JobHistory) update(user domain.User, permission domain.Permission, originalBody string) (generalResp *GeneralResp, status int, err error) {
	...
	if history.StatusID > 0 && len(history.JobHistID) > 0 {
		_, _, err = Ms.UpdateJobHistory(history.JobHistID, history.StatusID, history.ConfigID, history.Payload, sord(user.Username()))
		if err == nil {
			status = http.StatusOK
			generalResp.Response = fmt.Sprintf("job history %s updated", history.JobHistID)
		} else {
			err = errors.Errorf("error while updating job history - %s", err.Error())
		}
	} else {
		err = errors.New("both a \"status\" and a \"history_id\" greater than zero must be provided in the apiRequest body")
	}
